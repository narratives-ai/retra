...
Reports use ordinary Markdown headings without internal HTML tags: the report
is `#`, sections are `##`, and project-root folders are `###`. Every populated
section is grouped into contiguous project blocks, so outcomes, decisions,
friction, open work, and next steps from `Narratives`, `GemmatoDeffusion`, and
other projects do not mix.
